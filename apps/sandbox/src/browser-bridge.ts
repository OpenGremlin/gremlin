  ...
  const server = createServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      if (method === "GET" && url === "/browser/status") {
        ...
        sendJson(res, 200, await handleNavigate(await readBody(req)));
      } else if (method === "POST" && url === "/browser/screenshot") {
        sendJson(res, 200, await handleScreenshot(await readBody(req)));
      } else if (method === "POST" && url === "/browser/click") {
        sendJson(res, 200, await handleClick(await readBody(req)));
      } else if (method === "POST" && url === "/browser/type") {
        sendJson(res, 200, await handleType(await readBody(req)));
      } else if (method === "POST" && url === "/browser/evaluate") {
        sendJson(res, 200, await handleEvaluate(await readBody(req)));
      } else if (method === "GET" && url === "/browser/content") {
        sendJson(res, 200, await handleContent());
      } else {
